...
public class LaporanDataSiswa extends javax.swing.JFrame {

    public LaporanDataSiswa() {
        initComponents();
        LaporanSupport.susunLaporan(this, "Laporan Data Siswa", txtCari, table, btnHome, btnGuru, btnSiswa, btnJadwal, btnNilai, btnCari, btnRefresh, btnCetak);
        tampilData("");
    }
    ...
    private void tampilData(String keyword) {
        ...
        try {
            ...
            if (!keyword.trim().isEmpty()) {
                sql += " WHERE nis LIKE '%" + keyword + "%' OR nama LIKE '%" + keyword + "%'";
            }
            ResultSet rs = st.executeQuery(sql);
            int no = 1;
            while (rs.next()) {
                model.addRow(new Object[]{no++, rs.getString("nis"), rs.getString("nama"), rs.getString("jurusan"), rs.getString("kelas"), rs.getString("agama"), rs.getString("JenisKelamin")});
            }
    ...
    private void initComponents() {
        ...
        table.setModel(new DefaultTableModel(new Object[][]{}, new String[]{"No", "NIS", "Nama", "Jurusan", "Kelas", "Agama", "Jenis Kelamin"}));
        ...
        btnSiswa.addActionListener(evt -> tampilData(txtCari.getText()));
        ...
        btnCetak.addActionListener(evt -> LaporanSupport.cetakLaporan(this, table, "Laporan Data Siswa"));
